 ...
 * 主要获取聚合数据
 */
class WelcomeController extends HomelogedController
{
	...
	public function indexSearchReturnToDatatables()
	{
		...
		if ($listcount == 0) {
			...
			$this->display('Welcome:Welcome', 'UTF-8');
		} else {
    ...
    public function welcome() { 
        $user = M('Mzsjxxb');
        $condition['id'] = I('get.id');
        $iteminfo = $user->where($condition)->find();
        //var_dump($iteminfo);
        $this->assign('nowpage', cookie('nowpage'));
		...
		$this->assign('curPage', $curPage);
		/* if(empty($controller)){
			$this->display('Mzsj:mzsjdetail', 'UTF-8');
		}else{
			$this->display('Mzsj:mzsjyhzdetail', 'UTF-8');
		} */
		$this->display('Mzsj:mzsjdetail', 'UTF-8');
